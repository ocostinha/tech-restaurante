...
public class RestaurantController {
	...
	public RestaurantResponseDTO createRestaurant(@Valid @RequestBody RestaurantRequestDTO requestDTO) {
		return mapper.toResponse(createRestaurantUseCase.execute(mapper.toDomain(requestDTO)));
	}
	...
	public RestaurantResponseDTO editRestaurant(@PathVariable Long id,
			@Valid @RequestBody RestaurantRequestDTO requestDTO) {
		return mapper.toResponse(updateRestaurantUseCase.execute(id, mapper.toDomain(requestDTO)));
	}

	@Operation(summary = "Consultar restaurantes")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Restaurante localizados",
			content = { @Content(mediaType = "application/json",
					schema = @Schema(implementation = RestaurantResponseDTO.class)) }) })
	@GetMapping("/get")
	@ResponseStatus(HttpStatus.OK)
	public List<RestaurantResponseDTO> findRestaurants(@RequestParam(required = false) String name,
			@RequestParam(required = false) String neighborhood, @RequestParam(required = false) String city,
			@RequestParam(required = false) String state, @RequestParam(required = false) String cuisineType) {

		return findRestaurantUseCase.execute(name, neighborhood, city, state, cuisineType)
			.stream()
	...
	public RestaurantResponseDTO getRestaurant(@PathVariable Long id) {
		return mapper.toResponse(findRestaurantByIdUseCase.execute(id));
	}

	@Operation(summary = "Deletar restaurante")
	@ApiResponses(value = { @ApiResponse(responseCode = "202", description = "Restaurante deletado com sucesso"),
			@ApiResponse(responseCode = "422", description = "Erro na regra de negócio") })
	@DeleteMapping("/{id}")
	@ResponseStatus(HttpStatus.OK)
